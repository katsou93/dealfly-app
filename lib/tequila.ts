...
import { getAirlineColor, getAirlineName, formatDateForApi, addDays } from './utils'

const TEQUILA_BASE = 'https://tequila.kiwi.com'

export function buildSearchUrl(origin: string, params: SearchParams): string {
  const today = new Date()
  // For 16-month flex: search full 16-month window, high price limit to get all deals
  const is16months = params.flexMode === 'any16months'
  const dateFrom = params.departDateFrom || formatDateForApi(today)
  const dateTo = is16months
    ? formatDateForApi(addDays(today, 487))
    : params.departDateTo || formatDateForApi(addDays(today, 90))

  const maxPrice = is16months ? '2000' : String(
    params.tripType === 'package' ? params.maxPricePackage : params.maxPriceFlight
  )
  ...
  if (params.tripType === 'roundtrip') {
    ...
    const retTo = params.returnDateTo || formatDateForApi(addDays(today, is16months ? 487 : 30))
...
export function normalizeDeals(raw: any[], origin: string): FlightDeal[] {
  ...
  return raw.map(item => {
    ...
    return {
      ...
      returnDate: item.aTime ? new Date((item.aTime ?? 0) * 1000).toISOString() : undefined,
